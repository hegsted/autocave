...
if (lineDistance(x,y, 500, 500) > 499) {chance_of_on = 0;}
...
if (lineDistance(x,y, 500, 500) > 450 && lineDistance(x,y, 500, 500) < 499  ) {
grid[x][y].on = 1;
}
...
new_bullet(guy.x, guy.y, new_rotation, guy.gun.multi ,guy.gun.size, "color", guy.gun.deviance, guy.gun.speed, guy.gun.damage, guy.gun.graphic,guy.gun.lifespan,guy.gun.wallspan,guy.gun.wallspeed);
...
remove = 0;
bullets[s].age++
if(bullets[s].age > bullets[s].lifespan) { remove = 1; }
...
if (bullets[s].rotation > -1 && bullets[s].rotation < 0) { bullets[s].rotation = (Math.random() * 3);  } // + min
if (bullets[s].rotation > 0 && bullets[s].rotation < 0) { bullets[s].rotation = 0-(Math.random() * 3);  } // + min
if (bullets[s].rotation > 1 && bullets[s].rotation < 4) { bullets[s].rotation = 0-Math.random();  } // + min
if (bullets[s].rotation > -4 && bullets[s].rotation < -1) { bullets[s].rotation = Math.random();  }

 }

} else {

console.log(bullets[s].rotation);
...
if (keys.w.on == 1) {guy.yvel -= guy.speed;}

if (keys.a.on == 1) {guy.xvel -= guy.speed;}


if (keys.s.on == 1) {guy.yvel += guy.speed;}

if (keys.d.on == 1) {guy.xvel += guy.speed;}
